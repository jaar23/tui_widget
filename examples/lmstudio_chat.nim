...
let toolSpecs = %*[
  {
    ...
    "function": {
      "name": "run_command",
      "description": "Run a shell command on the host and return its " &
                     "stdout+stderr. Use sparingly and avoid destructive ops.",
      "parameters": {
        "type": "object",
        "properties": {
  ...
  },
  {
    ...
    "function": {
      "name": "web_search",
      "description": "Search the web via DuckDuckGo. Returns a short " &
                     "summary plus the top related topics.",
      "parameters": {
        "type": "object",
        "properties": {
  ...
  },
  {
    ...
    "function": {
      "name": "write_file",
      "description": "Write text to a file under /tmp. The filename's path " &
                     "components are stripped; only the basename is used.",
      "parameters": {
        "type": "object",
        "properties": {
          "filename": {"type": "string", "description": "file basename"},
...
proc stripHtml(s: string): string =
  ## Strip tags + decode the handful of HTML entities DuckDuckGo emits.
...
proc execWebSearch(args: JsonNode): string =
  ## DuckDuckGo HTML scrape — the instant-answer JSON API only handles
  ## trivia/topic queries (e.g. returns nothing for "IBM stock price"), so
  ## we hit the HTML endpoint and extract titles + snippets for the top
  ## results. Brittle if DDG changes markup, but adequate for a demo.
  try:
    ...
    defer: client.close()
    let url  = "https://html.duckduckgo.com/html/?q=" & encodeUrl(q)
    ...
    var i = 0
    while n < 4:
      let cls = html.find("class=\"result__a\"", i)
      if cls < 0: break
      let openGT = html.find('>', cls)
      let closeA = html.find("</a>", openGT)
      if openGT < 0 or closeA < 0: break
      let title = stripHtml(html[openGT + 1 ..< closeA])

      # Look for the snippet belonging to THIS result: it sits between
      # closeA and the next `result__a` (the next result block's title).
      let nextResultA = html.find("class=\"result__a\"", closeA + 4)
      let bound  = if nextResultA > 0: nextResultA else: html.len
      let snipCls = html.find("class=\"result__snippet\"", closeA)
      var snippet = ""
      if snipCls > 0 and snipCls < bound:
        let sOpenGT = html.find('>', snipCls)
        let sCloseA = html.find("</a>", sOpenGT)
        if sOpenGT > 0 and sCloseA > 0:
          snippet = stripHtml(html[sOpenGT + 1 ..< sCloseA])

      if title.len > 0:
        inc n
        lines.add($n & ". " & title)
        if snippet.len > 0:
          # Trim very long snippets so the LLM context stays sane.
          let snip = if snippet.len > 240: snippet[0 ..< 240] & "…" else: snippet
          lines.add("   " & snip)
      i = closeA + 4
...
proc execWriteFile(args: JsonNode): string =
  try:
    ...
    if filename.len == 0:
      return "write_file error: empty filename"
    let path = "/tmp" / filename
    writeFile(path, content)
    result = "wrote " & $content.len & " bytes to " & path
  except CatchableError:
    result = "write_file error: " & getCurrentExceptionMsg()

proc execTool(name: string, args: JsonNode): string =
  case name
